...
def pull_motion_event(queuename):
    ...
    if s:
        ...
        motion_event.send(__name__,data=json.loads(s))

    Timer(30,pull_motion_event,args=[settings.MOTION_REDIS,]).start()
...
class MotionControlConfig(AppConfig):
    ...
    def ready(self):
        if 'runserver' in sys.argv:
            if settings.MOTION_REDIS:
                # enable redis poller for motion events
                from threading import Timer
                Timer(10,pull_motion_event,args=[settings.MOTION_REDIS,]).start()
